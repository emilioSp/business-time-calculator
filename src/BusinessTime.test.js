...
import { BusinessTime } from './BusinessTime.js';
import { describe, test } from 'node:test';
import * as assert from 'node:assert/strict';
const testEachComputeTime = (testCases, businessTimeFunctionName) => {
...
const testEachMoveDateInBusinessTime = (testCases) => {
    for (const { businessTimezone, businessHours, businessDays, holidays, datetime, moveBehind, expected, } of testCases) {
        ...
        assert.deepStrictEqual(businessTime
...
const testEachIsBusinessDay = (testCases) => {
...
const testEachAddBusinessSecondsToDate = (testCases) => {
...
const testEachRemoveBusinessSecondsToDate = (testCases) => {
    for (const { seconds, businessTimezone, businessHours, businessDays, holidays, datetime, expected, } of testCases) {
        const datetimeObj = DateTime.fromISO(datetime);
        if (!datetimeObj.isValid) {
            throw new Error(`Invalid datetime: ${datetime}`);
